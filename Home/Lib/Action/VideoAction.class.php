...
class VideoAction extends CommonAction{	
	function videolist(){
		...
		$count=$video->where($wherev)->count();// 查询满足要求的总记录数
		$page=new Page($count,9);// 实例化分页类 传入总记录数和每页显示的记录数
		$page->setConfig('header','条记录');//修好显示内容
		$show=$page->show();// 分页显示输出
		$arr=$video->where($wherev)->limit($page->firstRow.','.$page->listRows)->select();
		foreach ($arr as $val){
	...
	function videoshow(){
		...
		$wherev['id']=$_GET['id'];
		$wherev['type']='1';
		$con=$video->where($wherev)->select();
		foreach ($con as $pl){
			$w=preg_split('[\.]',$pl['name']);
			$vi[]=array("id"=>"{$pl['id']}","name"=>"$w[0]","time"=>date('Y/m/d',$pl['time']),"url"=>"{$pl['url']}","remark"=>"{$pl['remark']}");
		}
		$this->assign('con',$vi);
